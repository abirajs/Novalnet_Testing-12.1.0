...
class PaymentService
{
            ...
            if(!is_null($basket) && $basket instanceof Basket && !empty($billingAddressId)) {
                ...
                if($this->settingsService->getPaymentSettingsValue('payment_active', $paymentKey) == true) {
                    ...
                    && ((int) $minimumInstalmentAmount <= (int) $basketAmount)) {
                        // If the guaranteed conditions are met, display the instalment payments
                        return true;
                    }
                    // If none matches, error message displayed
                    return false;
    ...
    */
	...
	if(in_array($transactionData['paymentName'], ['novalnet_instalment_invoice', 'novalnet_instalment_sepa'])) {
		$invoiceComments = PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_duedate_text'), $transactionData['cycle_amount']/100, $transactionData['currency'], date('Y/m/d', (int)strtotime($transactionData['due_date'])));
        // If the transaction is in On-Hold not displaying the due date
        if($transactionData['tx_status'] == 'ON_HOLD') {
        	$invoiceComments = PHP_EOL . PHP_EOL . sprintf($this->paymentHelper->getTranslatedText('transfer_amount_text'), $transactionData['cycle_amount']/100, $transactionData['currency']);
        }
	} else {
   ...
   public function getSavedPaymentDetails(&$paymentResponseData)
   {
    ...
    public function doInstalmentVoid($transactionData, $paymentUrl)
    {
	    ...
	    $paymentRequestData['custom']['shop_invoked'] = 1;
            ...
            $paymentResponseData = array_merge($paymentRequestData, $paymentResponseData);
            // Booking Message
            $paymentResponseData['bookingText'] = sprintf($this->paymentHelper->getTranslatedText('instalment_all_cycle_cancel', $transactionData['lang']), $paymentResponseData['transaction']['tid'], date('d-m-Y'), $paymentResponseData['transaction']['tid'], $paymentResponseData['transaction']['refund']['amount'] / 100, $paymentResponseData['transaction']['currency'], $paymentResponseData['transaction']['refund']['tid']);
